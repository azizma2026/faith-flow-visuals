...
const QuranWordTranslation: React.FC<WordTranslationProps> = ({ 
  ...
  if (!words || words.length === 0) {
    return (
      <p className={`text-right ${className} leading-loose`} dir="rtl">
        {arabicText}
      </p>
    );
  }
        ...
        {words.map((word, index) => (
          <Tooltip key={index}>
            <TooltipTrigger asChild>
              <span className="inline-block mx-1 cursor-help hover:text-islamic-gold transition-colors">
                {word.arabic}
              </span>
            </TooltipTrigger>
            <TooltipContent className="bg-card border-card-foreground/20 p-2 max-w-xs">
